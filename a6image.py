...
This modules contains a single class.  Instances of this
class support an image that can
...
class Image(object):
    ...
    So the call above is the same as the image call
     ...
     These operations are used by the greyscale filters and the
     stenography methods.
    """
    ...
    def setWidth(self,value):
        ...
        width*height == # of pixels. This can only happen if the
        value is valid.
        ...
        """
        assert type(value) == int and value >= 0, 'invalid width'
        assert type(len(self._data)%value) == int
        assert len(self._data)%value == 0, 'invalid height'
    ...
    def setHeight(self,value):
        ...
        width*height == # of pixels. This can only happen
        ...
        """
        assert type(value) == int and value >= 0, 'invalid height'
        assert type(value) == int
        assert len(self._data)%int(value) == 0, 'invalid height'
        self._height = value
    ...
    def __init__(self, data, width):
        ...
        Precondition: width is an int > 0 and evenly divides the
        length of pixels
        """
        assert _is_pixel_list(data) == True, 'invalid data'
        assert type(width)== int and width >= 0, 'invalid width'
        assert len(data)% width == 0, 'invalid height'
        self._data = data
        self._width = width
    ...
    def __getitem__(self, pos):
        ...
        Precondition: pos is an int and a valid position >= 0
         in the pixel list.
        """
        assert type(pos) == int and pos >= 0, 'invalid position'
        assert pos < len(self._data), 'position greater than length'
        return self._data[pos]    # Implement me
    ...
    def __setitem__(self, pos, pixel):
        ...
        assert type(pos) == int and pos >= 0, 'invalid position'
        assert pos < len(self._data), 'position greater than length'
        assert type(pixel) == tuple, 'wrong pixel format'
    ...
    def getPixel(self, row, col):
        ...
        copy = self._data[:]
        return copy[self._width * row + col]  # not sure
    ...
    def setPixel(self, row, col, pixel):
        ...
        assert type(row) == int and row >=0 and row < self._height
        assert type(col) == int and col >=0 and col<self._width
        assert _is_pixel(pixel), 'invalid pixel value'
        copy = self._data
        copy[self._width * row + col] = pixel
        # Implement me
