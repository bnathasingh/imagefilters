...
class Filter(a6editor.Editor):
    ...
    def reflectVert(self):
        ...
        for row in range(current.getHeight()//2):      # Loop over the rows
            for col in range(current.getWidth()):  # Loop over the columns
                k = current.getWidth()-1-row
                current.swapPixels(row,col,k,col)
    ...
    def monochromify(self, sepia):
        ...
        for pos in range(len(current)):
            ...
            brightness= 0.3*red + 0.6*green + 0.1*blue
            if sepia == True:
                greensep = int(0.6*brightness)
                bluesep = int(0.4*brightness)
                rgbsep = (int(brightness), greensep, bluesep)
                current[pos] = rgbsep
            else:
                rgbgray = (int(brightness), int(brightness), int(brightness))
                current[pos] = rgbgray
    ...
    def jail(self):
        ...
        * Put n 4-pixel vertical bars inside, where n is
        ...
        bars = (current.getWidth()-7)//50
        d = (current.getWidth() - (bars+2)*4)/(bars+1)
        for i in range(bars):
            self._drawVBar(int(round(4*(i+1)+d*(i+1))), (255, 0, 0))
    ...
    def vignette(self):
        ...
        """
        current = self.getCurrent()
        halfheigh=current.getHeight()/2
        halfwidth= current.getWidth()/2
        hfD = halfheigh*halfheigh + halfwidth*halfwidth
        #print("hfd is "+str(hfD))
        centerrow = current.getHeight()/2
        #print("center row is "+str(centerrow))
        centercol = current.getWidth()/2
        #print("center col is "+ str(centercol))
        for r in range(current.getHeight()):
            for c in range(current.getWidth()):
                rc=r-centerrow
                #print("rc is "+ str(rc))
                cc=c-centercol
                #print("cc is "+str(cc))
                d = rc*rc + cc*cc
                pixel = current.getPixel(r, c)
                red = pixel[0]
                green = pixel[1]
                blue = pixel[2]
                vig=1 - (d / hfD)
                #print("vig is"+str(vig))
                red2=int(red*vig)
                green2= int(green * vig)
                blue2=int(blue*vig)
                pixel= (red2, green2, blue2)
                current.setPixel(r, c, pixel)
    ...
    def _drawVBar(self, column, pixel):
        """ Draws a vertical bar on the current image at the the given column
        in the given color (in (red, green blue) form).

        This method draws a vertical 4 pixel-wide bar at the given column of
        the current image. This means that the bar includes column, column+1,
        column+2, and column+3. The bar uses the color given by the pixel
        value.

        Parameter column: The start of the column to draw the bar
        Precondition: column is an int, 0 <= column  &&  row+2 < image width

        Parameter pixel: The pixel color to use
        Precondition: pixel is a 3-element tuple (r,b,g) of ints in 0..255"""
        current = self.getCurrent()
        for row in range(current.getHeight()):
            current.setPixel(row, column, pixel)
            current.setPixel(row, column+1, pixel)
            current.setPixel(row, column+2, pixel)
            current.setPixel(row, column+3, pixel)
